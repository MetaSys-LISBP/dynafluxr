...
## Fist estimate of fluxes with `dynafluxr`
...
points(res$tp, res$mf$PYR, pch="o", cex=0.5)
```
There is some progress in fit quality but PYR is still not well fitted. We try to add next divergin metabolites to NA: BPG (a little bit buried in the plot) and ACCOA.
```{r}
...
## Resolving problem of overfitting

By default, the number of internal knots used for fitting is 5. Let try smaller values (1, 2 and 3) and see if the measurements are still well fitted. We will continue to use PYR for this purpose:
```{r}
res1=cli(c("-m", "glyco/data_PRO.tsv", "-s", "glyco/network_PRO.txt", "--decr", "GLC", "-o", "", "--lna", "FBP,F6P,BPG,ACCOA", "-k", "1"))
res2=cli(c("-m", "glyco/data_PRO.tsv", "-s", "glyco/network_PRO.txt", "--decr", "GLC", "-o", "", "--lna", "FBP,F6P,BPG,ACCOA", "-k", "2"))
res3=cli(c("-m", "glyco/data_PRO.tsv", "-s", "glyco/network_PRO.txt", "--decr", "GLC", "-o", "", "--lna", "FBP,F6P,BPG,ACCOA", "-k", "3"))
layout(t(1:3))
plot(res1$tpp, res1$isp(res1$tpp, "PYR"), main="PYR k=1", ylab="C", xlab="T", t="l")
points(res1$tp, res1$mf$PYR, pch="o", cex=0.5)
plot(res2$tpp, res2$isp(res2$tpp, "PYR"), main="PYR k=2", ylab="C", xlab="T", t="l")
points(res2$tp, res2$mf$PYR, pch="o", cex=0.5)
plot(res3$tpp, res$isp(res3$tpp, "PYR"), main="PYR k=3", ylab="C", xlab="T", t="l")
points(res3$tp, res3$mf$PYR, pch="o", cex=0.5)

```
The value `k=3` seems to provide satisfactory fit. The total data fit gives:
```{r}
res=res3
nm=colnames(res$mf)[-1L]
...
legend("topright", colnames(bsppar(res$vsp)$qw), lty=1:5, col=1:6, cex=0.75)
```
The curve oscillations have disappeared and we still have a good measurement fit with `k=3`. We can conclude that for `k=5` they were due to overfitting. The overall fit quality can be confirmed with $\chi^2$-test results:
```{r}
print(res$chi2tab)
```

Despite a slight variance increasing, the $p$-values are increased compared to `k=5` which is a good news. The $p$-value increased because of lowering the number of estimated parameters, i.e. increasing degrees of freedom from 180 to 182.
