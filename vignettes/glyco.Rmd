...
```{r}
res=cli(c("-m", "glyco/data.tsv", "-s", "glyco/network.txt", "--decr", "GLC", "--incr", "FBP", "--skip", "24"))
```

The same command could be run from a shell (not R) environment. In this case it would look like:

```
$ Rscript -e "dynafluxr::cli()" -m glyco/data.tsv -s glyco/network.txt --decr GLC --incr FBP --skip 24
```
...
```{r}
res=cli(c("-m", "glyco/data.tsv", "-s", "glyco/network.txt", "--decr", "GLC", "--incr", "FBP", "--skip", "24", "-o", ""))
```
...
```{r}
res=cli(c("-m", "glyco/data.tsv", "-s", "glyco/network.txt", "--decr", "GLC", "--incr", "FBP", "--skip", "24", "--sf=FBP,F6P", "-o", "", "--sderr=GLC=0.05,G6P=0.05,FBP=0.1,F6P=0.05"))
print(res$chi2tab)
```
